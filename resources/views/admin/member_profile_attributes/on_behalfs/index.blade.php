                    ...
                    <tbody>
                        @foreach($on_behalves as $key => $on_behalf)
                            <tr>
                                ...
                                <td class="text-right">
                                    @can('edit_on_behalf')
                                        <a href="javascript:void(0);" onclick="on_behalf_modal('{{ route('on-behalf.edit', encrypt($on_behalf->id) )}}')" class="btn btn-soft-primary btn-icon btn-circle btn-sm" title="{{ translate('Edit') }}">
